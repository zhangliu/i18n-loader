...
const genFile = async (fileType = 'json', filePath = '.', langs) => {
  ...
  // json 文件
  if (type === 'json') return fs.writeFileSync(i18nFile, JSON.stringify(langs, null, 4));

  // xlsx 文件
  const data = [['keys', 'source', 'zh']];
  Object.keys(langs).forEach(key => data.push([key, langs[key], langs[key]]));
